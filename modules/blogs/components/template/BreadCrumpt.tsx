...
const BreadCrumpt: NextPage<any> = ({data , category , page}) => {
    ...
    return (
        <div
            ...
            <Link href='/'>
                <Home className="w-5 opacity-30 hover:opacity-100"/>
            </Link><p className="text-gray-400">/</p>
            <Link href={page == "بلاگ" ? '/blog' : useRouter().query} className={`${data && 'text-gray-400'} hover:text-black`}>
                {page}
            </Link><p className={`text-gray-400 ${!data && 'hidden'}`}>/</p>
            {
                category &&
                <Link className={`text-gray-400 ${!data && 'hidden'} hover:text-black`} href={`/blog/category/${category[1]}`}>{category[0]}</Link>
